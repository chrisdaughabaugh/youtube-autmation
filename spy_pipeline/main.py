...
Usage:
  ...
  python main.py stage voice   --dir <output_folder>
...
# Resolve paths before importing config so .env loads correctly
...
def run_single(theme: str | None = None, out_dir: Path | None = None) -> Path:
    ...
    segments = generate_voiceover(story, out_dir)
    ...
    video_path = assemble_video(story, segments, frame_paths, out_dir)
...
def run_stage(stage: str, theme: str | None, dir_path: str | None) -> None:
    if stage == "story":
        ...
        if not dir_path:
            sys.exit("  --dir required for stage voice")
        out = Path(dir_path)
        story = json.loads((out / "story.json").read_text())
        generate_voiceover(story, out)
        ...
        out = Path(dir_path)
        story = json.loads((out / "story.json").read_text())
        paths = generate_visuals(story, out)
        for p in paths:
            print(p)
        ...
        assemble_video(story, segments, frame_paths, out)
        ...
        out = Path(dir_path)
        story = json.loads((out / "story.json").read_text())
        meta = generate_metadata(story, out)
        print(json.dumps(meta, indent=2))

    else:
        sys.exit(f"  Unknown stage: {stage}. Choose: story, voice, visuals, assemble, metadata")
...
def main():
    ...
    p_stage.add_argument("name", help="story | voice | visuals | assemble | metadata")
